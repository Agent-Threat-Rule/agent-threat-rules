...
2. Prevention Step 2: **Multi-Layer Detection with Pattern Matching:** Deploy detection rules that identify injection patterns across all input channels. ATR (Agent Threat Rules) publishes an open, MIT-licensed rule set for this category, including ATR-2026-00001 (Direct Prompt Injection), ATR-2026-00002 (Indirect Prompt Injection), ATR-2026-00003 (Jailbreak Attempt, with multilingual coverage), ATR-2026-00004 (System Prompt Override) and ATR-2026-00005 (Multi-Turn Injection); the current per-category rule counts are published in the project's `data/stats.json`. Measured against a self-built 850-sample PINT-format corpus (`deepset/prompt-injections` + `Lakera/gandalf_ignore_instructions`, **not** Lakera's official PINT benchmark), these rules reach 65.4% recall at 100.0% precision (ATR 3.5.12, measured 2026-08-15). That figure is a prompt-injection-family score on short prompt text, not a measure of overall coverage, and it should be read alongside the project's published limitations.
...
2. [ATR-2026-00001: Direct Prompt Injection](https://github.com/Agent-Threat-Rule/agent-threat-rules): Open-source detection rule with regex patterns for instruction override, task switching, and forget-all attacks.
3. [ATR-2026-00002: Indirect Prompt Injection](https://github.com/Agent-Threat-Rule/agent-threat-rules): 13-layer detection covering HTML comments, zero-width characters, model tokens, CSS-hidden text, and base64 payloads.
