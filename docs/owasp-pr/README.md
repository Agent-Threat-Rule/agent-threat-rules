...
- **PINT-format corpus** (850 samples from deepset + Lakera Gandalf, not Lakera's official PINT benchmark): 65.4% recall at 3.5.12. Precision withdrawn: an all-attack corpus has no negatives, so the figure was a script convention rather than a measurement.
...
- Eval: PINT-format corpus, self-built from public datasets (65.4% recall at 3.5.12; precision withdrawn -- all-attack corpus, no negatives)
- License: MIT
