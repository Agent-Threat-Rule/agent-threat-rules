...
- **ATR corpus version:** v3.5.8 HEAD, 825 rules across 10 detection-rule categories. Headline benchmarks below were measured at v3.5.0 (2026-06-16).
...
| R9 | **Scan local network for open or vulnerable MCP servers** (CSI names MCP Scanner, Ramparts, CyberMCP, Proximity) | ATR is the rule *content* those scanners consume (825 machine-readable rules as of 2026-09-22; see `data/stats.json` for the current count, and `ADOPTERS.md` for downstream repositories that have merged ATR rules, including the open-source cisco-ai-defense/skill-scanner) — **ATR is not itself a scanner and does no network discovery** | COMPLEMENTARY |
