...
- **ATR corpus version:** v3.5.8, 825 rules across 10 detection-rule categories
...
ATR ships 825 rules across 10 categories (as of 2026-09-22; `data/stats.json` carries the current count) with 6-framework compliance metadata (OWASP Agentic Top 10 + OWASP LLM Top 10 + MITRE ATLAS + NIST AI RMF + EU AI Act + ISO/IEC 42001), and has had rule contributions merged upstream into Microsoft Agent Governance Toolkit (PR #1277 / 287-rule weekly auto-sync + PR #908 + #1981 the copilot-swe-agent regression-test loop), Microsoft PyRIT (PR #1715 merged 2026-05-27 by Roman Lutz), Cisco AI Defense skill-scanner (PR #99 / 314 rules), MISP via CIRCL Luxembourg (galaxy PR #1207 / 336 rules + taxonomies PR #323), and precize/Agentic-AI-Top10-Vulnerability (third-party community repo, PR #14 + #18). This document maps each Five Eyes category to specific ATR rule clusters and is honest about what ATR does not cover.
...
This is exactly the position ATR occupies. ATR is the open, MIT-licensed, agentic-specific detection rule corpus that (a) is not LLM-vulnerability-focused like OWASP LLM Top 10 or MITRE ATLAS, (b) maps to OWASP Agentic Top 10 (the agent-specific successor framework), (c) has had rule contributions merged upstream into the repositories of Microsoft Agent Governance Toolkit (PR #1277 / 287-rule weekly auto-sync + PR #908 + #1981), Microsoft PyRIT (PR #1715 merged 2026-05-27 by Roman Lutz), Cisco AI Defense skill-scanner (PR #99 / 314 rules), MISP galaxy + taxonomies via CIRCL Luxembourg (PR #1207 / #323), and the precize/Agentic-AI-Top10-Vulnerability community repo (PR #14 + #18, third-party not OWASP Foundation). These are merges into open-source repositories with per-PR evidence recorded in `ADOPTERS.md`; they are not statements about any vendor's commercial product, and PR counts and rule-pack sizes are point-in-time.
...
- [x] Rule-ID list last enumerated at v3.5.0 / 652 rules / 2026-06-16 (header corpus count refreshed to 655 on 2026-06-29; rules added since are not yet individually back-referenced in the body).
