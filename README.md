...
[![Rules](https://img.shields.io/badge/rules-793-blue?style=flat-square)](#5-specification)
...
Raw data: [`data/full-scan-v2-2026-04-14.json`](data/full-scan-v2-2026-04-14.json) — 101,280 skills scanned, 1,434 flagged, engine v2.0.0 with 113 rules. Those are rule-hit counts, not adjudicated verdicts: the raw records carry `file`, `matches` and `source` and nothing else, so no confirmed-malicious figure is derivable from this scan and none is quoted. The 96,096 figure that appears in older write-ups is a narrower scope, not a competing count — it is the four SKILL.md registries alone (OpenClaw 56,480 + ClawHub 36,378 + Skills.sh 3,115 + Hermes 123), excluding MCP Registry server definitions. The narrative report in [`docs/research/openclaw-malware-campaign-2026-04.md`](docs/research/openclaw-malware-campaign-2026-04.md) predates that correction and still reads flag counts as confirmations.
